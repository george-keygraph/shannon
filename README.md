...
Claude Shannon founded information theory in 1948 and gave cryptography its first rigorous math a year later, so he is roughly the reason security is something you prove rather than assert. His maxim says to design every system on the assumption that the attacker already knows exactly how it works. That is the entire argument for open-source security tooling, made decades before anyone could star a repo, and we took it literally: we open-sourced the attacker. He also kept a box on his desk whose only function was to reach out and switch itself off. Rigorous about the math, unserious about everything else. We aim for that ratio.
